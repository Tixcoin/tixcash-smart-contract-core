...
contract('TixCashCore', async ([account1, account2]) => {
    let nodeIRaw;
    beforeEach(async () => {
...
contract('TixCashCore', async ([firstValidator, secondValidator, thirdValidator]) => {
    beforeEach(async () => {
        this.sfc = await UnitTestSFC.new();
...
contract('TixCashCore', async ([firstValidator, secondValidator, thirdValidator, firstDelegator, secondDelegator, thirdDelegator]) => {
    beforeEach(async () => {
        this.sfc = await UnitTestSFC.new();
        const nodeIRaw = await NodeDriver.new();
        const evmWriter = await StubEvmWriter.new();
...
contract('TixCashCore', async ([firstValidator, secondValidator, thirdValidator, testValidator, firstDelegator, secondDelegator, account1, account2, account3, account4]) => {
...
contract('TixCashCore', async ([firstValidator, firstDelegator]) => {
    let firstValidatorID;
...
contract('TixCashCore', async ([firstValidator, testValidator, firstDelegator, secondDelegator, thirdDelegator, account1, account2, account3]) => {
...
contract('TixCashCore', async ([firstValidator, testValidator, firstDelegator, secondDelegator, thirdDelegator, account1, account2, account3]) => {
    ...
    describe('Test Calculation Rewards with Lockup', () => {
        ...
        it('Should not unlock if not locked up TXH', async () => {
            await sealEpoch(this.sfc, (new BN(1000)).toString());
