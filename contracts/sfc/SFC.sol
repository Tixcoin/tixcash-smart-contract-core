...
contract SFC is Initializable, Ownable, StakersConstants, Version {
    ...
    event InflatedTXH(address indexed receiver, uint256 amount, string justification);
    ...
    */
    ...
    function undelegate(uint256 toValidatorID, uint256 wrID, uint256 amount) public {
        ...
        require(_checkAllowedToWithdraw(delegator, toValidatorID), "outstanding sTXH balance");
    ...
    function withdraw(uint256 toValidatorID, uint256 wrID) public {
        ...
        require(sent, "Failed to send TXH");
    ...
    // mintTXH allows SFC owner to mint an arbitrary amount of TXH tokens
    // justification is a human readable description of why tokens were minted (e.g. because ERC20 TXH tokens were burnt)
    function mintTXH(address payable receiver, uint256 amount, string calldata justification) onlyOwner external {
        _mintNativeToken(amount);
        receiver.transfer(amount);
        emit InflatedTXH(receiver, amount, justification);
    }
